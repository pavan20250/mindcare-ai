      ...
      visual: (
        <div className="flex flex-col gap-0.5 mt-2">
          <div className="flex items-start gap-1">
            <div className="w-3 h-3 sm:w-4 sm:h-4 rounded-full bg-blue-500/20 shrink-0 mt-0.5" />
            <div className="bg-blue-500/10 rounded rounded-tl-sm px-1.5 py-0.5 text-[8px] sm:text-[9px] text-blue-300 leading-tight">How are you feeling?</div>
          </div>
          <div className="flex items-start gap-1 justify-end">
            <div className="bg-white/[0.06] rounded rounded-tr-sm px-1.5 py-0.5 text-[8px] sm:text-[9px] text-slate-300 leading-tight">Anxious lately</div>
            <div className="w-3 h-3 sm:w-4 sm:h-4 rounded-full bg-white/10 shrink-0 mt-0.5" />
          </div>
        </div>
      ),
    },
        ...
        <div className="space-y-1 mt-2">
          {[
            ...
            <div key={label} className="flex items-center gap-1">
              <span className="text-[7px] sm:text-[8px] text-slate-500 w-6 sm:w-7 shrink-0 font-medium">{label}</span>
              <div className="flex-1 h-1 bg-white/[0.06] rounded-full overflow-hidden">
                <div className={`h-full rounded-full ${color}`} style={{ width }} />
              </div>
              <svg className="w-2 h-2 sm:w-2.5 sm:h-2.5 text-emerald-400 shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="flex items-center gap-1.5 mt-2 p-1.5 rounded-md bg-white/[0.03] border border-white/[0.06]">
          <div className="flex -space-x-1">
            {['bg-violet-400', 'bg-purple-400', 'bg-indigo-400'].map((bg, i) => (
              <div key={i} className={`w-4 h-4 sm:w-5 sm:h-5 rounded-full border-2 border-[#0f1729] ${bg}`} />
            ))}
          </div>
          <div className="min-w-0">
            <div className="text-[8px] sm:text-[9px] font-semibold text-slate-200 leading-tight">Care team matched</div>
            <div className="text-[7px] sm:text-[8px] text-slate-500">3 providers ready</div>
          </div>
        </div>
      ),
    },
  ...
  const features = [
    {
      ...
      visual: (
        <div className="bg-white/[0.06] backdrop-blur rounded-lg px-2.5 py-2 border border-white/[0.08]">
          ...
          <p className="text-[9px] text-slate-300 mb-2 leading-snug">Over the past week, how would you rate your energy levels?</p>
          <div className="flex gap-1 flex-wrap xs:flex-nowrap">
            {['Very low', 'Low', 'Moderate', 'Good', 'High'].map((opt, i) => (
              <div
                key={opt}
                className={`flex-1 min-w-0 rounded-md py-1 text-center text-[7px] border truncate ${
                  i === 1
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-[8px] text-slate-500 shrink-0">33%</span>
          </div>
        </div>
      ),
    },
          ...
          {[
            ...
            <div key={item.label} className={`flex items-center gap-2 ${item.bg} backdrop-blur rounded-lg px-2.5 py-1.5 border ${item.border}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className="text-[9px] text-slate-400 ml-1.5 truncate">{item.sublabel}</span>
              </div>
              <svg className="w-3 h-3 text-slate-500 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2">
          ...
          <div className="bg-white/[0.06] backdrop-blur rounded-lg px-2.5 py-2 border border-white/[0.08]">
            ...
            <div className="flex items-end gap-[3px] h-8 sm:h-9">
              {[65, 58, 52, 55, 42, 38, 30, 25].map((h, i) => (
                <div
                  key={i}
                  className="flex-1 rounded-sm bg-gradient-to-t from-violet-500/70 to-purple-400/80"
          ...
          <div className="flex gap-1.5">
            {[
              ...
              <div key={m.label} className="flex-1 bg-white/[0.06] rounded-lg px-1.5 py-1 border border-white/[0.08] text-center">
                <p className={`text-[11px] sm:text-[12px] font-bold ${m.color}`}>{m.val}</p>
        ...
        <div className="space-y-1.5">
          <div className="flex gap-1.5">
            <div className="flex-1 bg-white/[0.06] backdrop-blur rounded-lg px-2 py-1.5 border border-white/[0.08]">
              ...
              <p className="text-[13px] font-bold text-white leading-tight">1,247</p>
              <span className="text-[7px] text-emerald-400">+12% this week</span>
            </div>
            <div className="flex-1 bg-white/[0.06] backdrop-blur rounded-lg px-2 py-1.5 border border-white/[0.08]">
              <p className="text-[8px] text-slate-500">Sessions Today</p>
              <p className="text-[13px] font-bold text-white leading-tight">84</p>
          ...
          <div className="bg-white/[0.06] backdrop-blur rounded-lg px-2.5 py-2 border border-white/[0.08]">
            ...
            <div className="flex items-end gap-1 h-6 sm:h-7">
              {[40, 55, 35, 65, 50, 72, 80].map((h, i) => (
                <div
                  key={i}
                  className="flex-1 rounded-sm bg-gradient-to-t from-indigo-500/60 to-blue-400/80"
    ...
    <section id="features" className="section-padding relative overflow-hidden">
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-6 sm:mb-8 space-y-2">
          <span className="inline-flex items-center gap-1.5 px-3 sm:px-3.5 py-1.5 rounded-full bg-white/[0.06] text-teal-300 text-[10px] sm:text-xs font-semibold uppercase tracking-wider border border-white/[0.08]">
            How It Works
          </span>
        ...
        {/* Mobile: vertical stack | sm+: horizontal grid */}
        <div className="relative mb-6 sm:mb-8">
          ...
          <div className="sm:hidden absolute left-[22px] top-10 bottom-10 w-px bg-gradient-to-b from-blue-500/30 via-emerald-500/30 to-violet-500/30 z-0" />
          ...
          <div className="flex flex-col sm:grid sm:grid-cols-3 gap-3 sm:gap-4">
            {steps.map((step) => (
              <div
                key={step.num}
                className="relative z-10 group bg-white/[0.04] backdrop-blur-lg rounded-xl border border-white/[0.08] shadow-lg shadow-black/10 hover:bg-white/[0.07] transition-all duration-300"
              >
                {/* Mobile layout: horizontal icon + text header */}
                <div className="flex sm:block items-start gap-3 p-3 sm:p-3">
                  <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-gradient-to-br ${step.gradient} flex items-center justify-center shadow-md shrink-0 group-hover:scale-110 transition-transform duration-500`}>
                    {step.icon}
                  </div>
                  <div className="flex-1 min-w-0 sm:mt-2">
                    <div className="flex sm:block items-baseline gap-1.5">
                      ...
                      <h3 className="font-bold text-white text-sm sm:text-sm leading-tight">{step.title}</h3>
                    </div>
                    <p className="text-slate-400 text-[10px] mt-0.5">{step.desc}</p>
                    {/* Visual shown inline on mobile */}
                    <div className="sm:hidden">{step.visual}</div>
                  </div>
                </div>
                {/* Visual shown below on desktop */}
                <div className="hidden sm:block px-3 pb-3">{step.visual}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 mb-6 sm:mb-8">
          ...
          <span className="text-[9px] text-slate-500 font-semibold uppercase tracking-widest whitespace-nowrap">Powered by</span>
          <div className="flex-1 h-px bg-gradient-to-l from-transparent to-white/[0.08]" />
        </div>

        {/* 4 feature cards — single col mobile, 2-col sm+ */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          {features.map((f) => (
            <div
              key={f.title}
              className="group bg-white/[0.04] backdrop-blur-lg rounded-xl p-3 sm:p-4 border border-white/[0.08] hover:border-white/[0.12] shadow-lg shadow-black/10 hover:bg-white/[0.07] transition-all duration-300 hover:-translate-y-0.5"
            >
              <div className="flex items-center gap-2.5 mb-2.5">
                <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-gradient-to-br ${f.gradient} flex items-center justify-center shadow-md ${f.shadowColor} shrink-0 group-hover:scale-110 transition-transform duration-500`}>
                  {f.icon}
                </div>
                <div className="min-w-0">
                  <h3 className="font-bold text-white text-xs sm:text-sm leading-tight">{f.title}</h3>
                  <p className="text-slate-400 text-[10px] sm:text-xs mt-0.5 leading-snug">{f.desc}</p>
                </div>
              </div>
              {f.visual}
            </div>
