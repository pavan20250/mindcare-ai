...
export interface Appointment {
  ...
  date: string; timeSlot: string; reason?: string; status?: 'confirmed' | 'cancelled'; createdAt: number;
}
...
export function AppointmentsView() {
  ...
  const fetchAppointments = useCallback(() => {
    ...
    fetch('/api/appointments', { credentials: 'include' })
      ...
      .catch(() => setAppointments([])).finally(() => setAppointmentsLoading(false));
  }, []);

  useEffect(() => {
    const url = specialtyFilter ? `/api/doctors?specialty=${encodeURIComponent(specialtyFilter)}` : '/api/doctors';
    setLoading(true);
  ...
  useEffect(() => {
    if (!bookingDoctor?.id || !bookingDate) { setSlots([]); setSelectedSlot(''); return; }
  ...
  const openBooking = (doctor: Doctor) => {
    setBookingDoctor(doctor); setBookingDate(''); setSlots([]);
    setSelectedSlot(''); setReason(''); setBookingSuccess(null);
  };
  const closeBooking = () => {
    setBookingDoctor(null); setBookingDate(''); setSelectedSlot(''); setReason(''); setBookingSuccess(null);
  };
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      const res = await fetch('/api/appointments', {
        ...
        body: JSON.stringify({ doctorId: bookingDoctor.id, doctorName: bookingDoctor.name, specialty: bookingDoctor.specialty, date: bookingDate, timeSlot: selectedSlot, reason: reason.trim() || undefined }),
      });
    ...
    <PageBackground>
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="mb-5">
          ...
          <p className="text-slate-500 text-sm mt-0.5">View your upcoming sessions and book with a provider that fits your needs.</p>
        </div>
              ...
              <CardContent className="pt-0">
                {appointmentsLoading ? (
                  <div className="space-y-3">
                    {[1, 2, 3].map((i) => (
                      <div key={i} className="flex gap-3 p-3 rounded-xl bg-white/30 border border-white/50">
                        <Skeleton className="h-10 w-10 rounded-lg shrink-0 bg-white/40" />
                  ...
                  <div className="rounded-2xl border border-dashed border-white/60 bg-white/20 p-6 text-center">
                    <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-white/30 border border-white/50 text-slate-400 mb-3">
                          ...
                          <CardContent className="p-0">
                            <div className="flex">
                              ...
                              <div className="flex-1 min-w-0 p-3">
                                ...
                                {apt.reason && (
                                  <p className="text-slate-400 text-xs mt-1 truncate" title={apt.reason}>{apt.reason}</p>
                                )}
                                <div className="flex items-center gap-2 mt-2">
                                  <Badge variant="secondary" className="text-[10px] font-semibold text-teal-700 bg-white/40 border border-teal-200/60 backdrop-blur-sm">
                                    Confirmed
                                  </Badge>
                                  {isUpcoming(apt.date) && (
                                    <Button type="button" variant="ghost" size="sm"
                                      className="h-6 text-xs text-slate-400 hover:text-red-600 hover:bg-red-50/60 px-1"
          ...
          <section className="lg:col-span-2 order-1 lg:order-2">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
              ...
              <Select value={specialtyFilter || 'all'} onValueChange={(v) => setSpecialtyFilter(v === 'all' ? '' : v)}>
                <SelectTrigger className="w-[180px] h-9 text-sm bg-white/40 border-white/60 backdrop-blur-sm text-slate-600 rounded-xl">
                  <SelectValue placeholder="Filter by specialty" />
                </SelectTrigger>
                <SelectContent className="border-white/50 bg-white/80 backdrop-blur-xl text-slate-600 rounded-xl">
                  ...
                  <Card key={i} className="overflow-hidden">
                    <CardContent className="p-5">
                      <div className="flex gap-4">
                        <Skeleton className="h-14 w-14 rounded-xl shrink-0 bg-white/40" />
                        <div className="flex-1 space-y-2">
                          <Skeleton className="h-5 w-32 bg-white/40" />
                          <Skeleton className="h-4 w-24 bg-white/40" />
                          <Skeleton className="h-12 w-full mt-2 bg-white/40" />
                          <Skeleton className="h-9 w-28 mt-3 bg-white/40" />
                        </div>
                      </div>
                    </CardContent>
                  </Card>
                        ...
                        <div className="flex">
                          ...
                          <div className="flex-1 min-w-0 p-4 sm:p-5">
                            <div className="flex gap-3 sm:gap-4">
                              <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-white/40 border border-white/60 text-teal-700 font-semibold text-sm">
                            ...
                            <Button onClick={() => openBooking(doctor)} size="sm"
                              className="mt-4 w-full sm:w-auto bg-teal-600 hover:bg-teal-700 text-white border-0 rounded-xl shadow-sm shadow-teal-600/20">
                              Book appointment
                            </Button>
                          </div>
                        </div>
        ...
        <div className="mt-6 flex justify-center">
          <Button variant="ghost" size="sm" asChild className="text-slate-400 hover:text-teal-600 hover:bg-white/30 rounded-xl">
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2 text-slate-900">
                  <span className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-50/60 border border-emerald-200/60 text-emerald-600">
              ...
              <div className="rounded-2xl border border-white/50 bg-white/30 p-4 text-sm backdrop-blur-sm">
                <p className="font-semibold text-slate-800">{bookingSuccess.doctorName}</p>
                <p className="text-teal-700">{bookingSuccess.specialty}</p>
                <p className="mt-2 text-slate-600">{formatDate(bookingSuccess.date)} at {formatTime(bookingSuccess.timeSlot)}</p>
                {bookingSuccess.reason && <p className="mt-1 text-slate-500">Reason: {bookingSuccess.reason}</p>}
            ...
            <>
              <DialogHeader>
                <div className="flex items-center gap-3">
                  <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-white/40 border border-white/60 text-teal-700 font-semibold text-sm">
              ...
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  ...
                  <Input type="date" min={minDate} value={bookingDate} onChange={(e) => setBookingDate(e.target.value)}
                    className="h-10 bg-white/50 border-white/60 text-slate-800 rounded-xl backdrop-blur-sm" />
                </div>
                <div className="space-y-2">
                  ...
                  {slotsLoading ? (
                    ...
                    <Select value={selectedSlot} onValueChange={setSelectedSlot}>
                      <SelectTrigger className="h-10 w-full bg-white/50 border-white/60 text-slate-700 rounded-xl backdrop-blur-sm">
                        <SelectValue placeholder="Choose a time" />
                      </SelectTrigger>
                      <SelectContent className="border-white/50 bg-white/80 backdrop-blur-xl text-slate-700 rounded-xl">
                        {slots.map((slot) => (
                ...
                <div className="space-y-2">
                  ...
                  <Textarea value={reason} onChange={(e) => setReason(e.target.value)}
                    placeholder="e.g. Follow-up from intake, depression screening" rows={2}
                    className="resize-none bg-white/50 border-white/60 text-slate-800 placeholder:text-slate-400 rounded-xl backdrop-blur-sm" />
                </div>
              </div>
              <DialogFooter className="gap-2 sm:gap-0">
