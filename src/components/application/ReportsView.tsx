...
const MOOD_META: Record<MoodValue, { label: string; icon: typeof Sun; score: number; color: string; bg: string; dot: string }> = {
  great:      { label: 'Great',      icon: Sun,       score: 5, color: 'text-amber-500',   bg: 'bg-amber-50',   dot: 'bg-amber-400'   },
  good:       { label: 'Good',       icon: Smile,     score: 4, color: 'text-emerald-500', bg: 'bg-emerald-50', dot: 'bg-emerald-400' },
  okay:       { label: 'Okay',       icon: Meh,       score: 3, color: 'text-sky-500',     bg: 'bg-sky-50',     dot: 'bg-sky-400'     },
  low:        { label: 'Low',        icon: CloudRain, score: 2, color: 'text-slate-500',   bg: 'bg-slate-50',   dot: 'bg-slate-400'   },
  struggling: { label: 'Struggling', icon: Frown,     score: 1, color: 'text-rose-500',    bg: 'bg-rose-50',    dot: 'bg-rose-400'    },
};

const INTAKE_LABELS: Record<string, string> = {
  welcome:        'Initial concerns',
  symptoms:       'Duration of symptoms',
  severity:       'Daily impact (1–10)',
  phq9:           'Interest / pleasure (PHQ-9)',
...
const SEVERITY_LABEL: Record<string, { label: string; color: string; bg: string }> = {
  '1': { label: 'Minimal', color: 'text-emerald-600', bg: 'bg-emerald-50' },
  '2': { label: 'Minimal', color: 'text-emerald-600', bg: 'bg-emerald-50' },
  '3': { label: 'Minimal', color: 'text-emerald-600', bg: 'bg-emerald-50' },
  '4': { label: 'Mild',    color: 'text-sky-600',     bg: 'bg-sky-50'     },
  '5': { label: 'Mild',    color: 'text-sky-600',     bg: 'bg-sky-50'     },
  '6': { label: 'Moderate', color: 'text-amber-600',  bg: 'bg-amber-50'   },
  '7': { label: 'Moderate', color: 'text-amber-600',  bg: 'bg-amber-50'   },
  '8': { label: 'High',    color: 'text-orange-600',  bg: 'bg-orange-50'  },
  '9': { label: 'Severe',  color: 'text-rose-600',    bg: 'bg-rose-50'    },
  '10':{ label: 'Severe',  color: 'text-rose-600',    bg: 'bg-rose-50'    },
};
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.12em]">{children}</p>
  );
}
  ...
  return (
    <div className="flex flex-col items-center gap-1.5 flex-1">
      <div className="relative w-full flex items-end justify-center" style={{ height: 56 }}>
        <div className="w-5 rounded-t-md bg-slate-100 overflow-hidden" style={{ height: '100%' }}>
          <div
            className={`w-full rounded-t-md transition-all duration-500 ${meta?.dot ?? 'bg-slate-200'}`}
...
function EmptyState({ message, href, cta }: { message: string; href: string; cta: string }) {
  return (
    <div className="flex flex-col items-center justify-center py-8 text-center gap-3">
      ...
      <Button asChild size="sm" className="h-8 text-xs bg-teal-600 hover:bg-teal-700 text-white border-0 shadow-sm shadow-teal-600/20">
...
export function ReportsView() {
  ...
  const avgMoodScore = useMemo(() => {
    ...
    const avg = scored.reduce((s, v) => s + MOOD_META[v].score, 0) / scored.length;
    return avg;
  }, [days7, moodMap]);
  ...
  const upcomingAppts = useMemo(() =>
    appointments.filter((a) =>
      a.status !== 'cancelled' &&
      new Date(`${a.date}T${a.timeSlot}`).getTime() >= Date.now()
    ).sort((a, b) =>
  ...
  if (loading) {
    return (
      <div className="min-h-full bg-[#f8fafb] flex items-center justify-center py-20">
  ...
  return (
    <div className="min-h-full bg-[#f8fafb]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-5">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 text-sm mt-0.5">
              A snapshot of your wellbeing, assessments, and care activity.
            </p>
          </div>
          <Button
            variant="outline"
            size="sm"
            className="h-8 text-xs border-slate-200 text-slate-600 hover:border-teal-300 hover:text-teal-700 hover:bg-teal-50 shrink-0"
        ...
        {/* ── Tabs ── */}
        <div className="flex gap-1 bg-white border border-slate-200/80 rounded-xl p-1 shadow-sm">
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              <StatCard
                icon={Heart}
                label="Avg mood (7d)"
                value={avgMoodLabel?.label ?? '—'}
                sub={avgMoodScore ? `Score ${avgMoodScore.toFixed(1)} / 5` : 'No data yet'}
                iconClass="bg-rose-50 text-rose-500"
                trend={avgMoodScore ? (avgMoodScore >= 3.5 ? 'up' : avgMoodScore < 2.5 ? 'down' : 'flat') : undefined}
              />
              <StatCard
                icon={Activity}
                label="Mood streak"
                value={moodStreak ? `${moodStreak}d` : '—'}
                sub={moodStreak > 0 ? 'Keep it up!' : 'Log your first'}
                iconClass="bg-amber-50 text-amber-500"
                trend={moodStreak >= 3 ? 'up' : undefined}
              />
              <StatCard
                icon={Calendar}
                label="Appointments"
              ...
              <StatCard
                icon={Brain}
                label="Assessment"
                value={intake ? 'Done' : 'Pending'}
                sub={intake?.completedAt ? formatDate(new Date(intake.completedAt).toISOString().slice(0, 10)) : 'Not started'}
                iconClass={intake ? 'bg-teal-50 text-teal-600' : 'bg-slate-100 text-slate-400'}
                trend={intake ? 'up' : undefined}
              />
            </div>
            ...
            <Card className="border-slate-200/80 shadow-sm">
              <CardContent className="p-5">
                ...
                <div className="flex gap-2 items-end">
                  {days7.map((date) => (
                    <MoodBar key={date} date={date} entry={moodMap[date]} />
                  ))}
                </div>
                {moods.length === 0 && (
                  <p className="text-[12px] text-slate-400 text-center mt-3">
                    No mood data yet. Log your mood from the Dashboard.
                  </p>
                )}
              </CardContent>
            </Card>

            {/* Clinical + upcoming side by side */}
              ...
              <Card className="border-slate-200/80 shadow-sm">
                <CardContent className="p-5">
                  ...
                  {intake ? (
                    <div className="space-y-3">
                      ...
                      {phq9Score !== null && (
                        <div className="flex items-center justify-between py-2 px-3 rounded-lg bg-slate-50 border border-slate-100">
                          ...
                          <Badge className="text-[10px] px-2 py-0.5 bg-violet-50 text-violet-700 border-0 font-bold">
                            Score {phq9Score}/3
                          </Badge>
                        </div>
                      )}
                      ...
                      <button
                        ...
                        className="text-[11px] text-teal-600 hover:text-teal-700 font-semibold flex items-center gap-0.5 mt-1"
                      >
                    ...
                    <EmptyState
                      message="Complete your clinical intake to see your assessment report."
                      href="/demo"
                      cta="Start intake"
                    />
                  )}
                  ...
                  {upcomingAppts.length > 0 ? (
                    <div className="space-y-2">
                      {upcomingAppts.slice(0, 3).map((appt) => (
                        <div key={appt.id} className="flex items-center gap-3 py-2 px-3 rounded-lg bg-slate-50 border border-slate-100">
            ...
            <Card className="border-slate-200/80 shadow-sm">
              <CardContent className="p-5">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <Badge className="ml-auto text-[10px] px-2 py-0.5 bg-slate-50 text-slate-500 border border-slate-200 shadow-none font-semibold">
                    {moodStreak}d streak
                  </Badge>
                </div>
                ...
                {moods.length > 0 ? (
                  <div className="mt-3 space-y-1.5">
                    {[...moods].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 14).map((entry) => {
                      ...
                      return (
                        <div key={entry.date} className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-slate-50 transition-colors">
                  ...
                  <EmptyState
                    message="You haven't logged any moods yet. Log daily from the Dashboard."
                    href="/dashboard"
                    cta="Go to dashboard"
                  />
                )}
          ...
          <div className="space-y-4">
            {intake ? (
                ...
                <Card className="border-slate-200/80 shadow-sm overflow-hidden">
                  ...
                  <CardContent className="p-5">
                          ...
                          {intake.completedAt && (
                            <div className="flex items-center gap-1.5 mt-0.5">
                              ...
                              <p className="text-[11px] text-slate-400">
                                {new Date(intake.completedAt).toLocaleDateString('en-US', { dateStyle: 'full' })}
                              </p>
                            </div>
                          )}
                        </div>
                      </div>
                      <Badge className="text-[9px] px-2 py-0.5 bg-teal-50 text-teal-700 border border-teal-200 font-bold shadow-none gap-1 shrink-0">
                    ...
                    <div className="flex flex-wrap gap-2 mt-4">
                      {severityMeta && (
                        <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-semibold ${severityMeta.bg} ${severityMeta.color}`}>
                          <Activity className="size-3" />
                          Impact: {severityMeta.label} ({severityRating}/10)
                        </div>
                      )}
                      {phq9Score !== null && (
                        <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-semibold bg-violet-50 text-violet-700">
                ...
                <Card className="border-slate-200/80 shadow-sm">
                  <CardContent className="p-5">
                    ...
                    <div className="divide-y divide-slate-100">
                      {Object.entries(intake.responses)
                        .filter(([, v]) => v && v !== '')
                        .map(([stepId, value]) => (
              ...
              <Card className="border-slate-200/80 shadow-sm">
                <CardContent className="p-5">
                  <EmptyState
                    message="Complete the clinical intake to generate your assessment report."
                    href="/demo"
                    cta="Start clinical intake"
                  />
                </CardContent>
              </Card>
            )}
            ...
            <Card className="border-slate-200/80 shadow-sm">
              <CardContent className="p-5">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <Badge className="ml-auto text-[10px] px-1.5 py-0 bg-blue-50 text-blue-600 border border-blue-100 shadow-none font-bold">
                ...
                {upcomingAppts.length > 0 ? (
                  <div className="space-y-2">
                    {upcomingAppts.map((appt) => (
                      <div key={appt.id} className="flex items-center gap-3 py-2.5 px-3 rounded-lg border border-slate-100 bg-slate-50">
                        ...
                        <Badge className="text-[9px] px-1.5 py-0 bg-emerald-50 text-emerald-700 border border-emerald-100 shadow-none font-bold shrink-0">
                          Confirmed
                        </Badge>
                      </div>
                    ))}
                ...
                <CardContent className="p-5">
                  ...
                  <div className="space-y-2">
                    {completedAppts.slice(0, 5).map((appt) => (
                      <div key={appt.id} className="flex items-center gap-3 py-2 px-3 rounded-lg hover:bg-slate-50 transition-colors">
