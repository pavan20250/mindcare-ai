...
const MOOD_OPTIONS = [
  { value: 'great' as const, label: 'Great', icon: Sun,       accent: 'text-amber-500' },
  { value: 'good'  as const, label: 'Good',  icon: Smile,     accent: 'text-emerald-500' },
  { value: 'okay'  as const, label: 'Okay',  icon: Meh,       accent: 'text-sky-500' },
  { value: 'low'   as const, label: 'Low',   icon: CloudRain, accent: 'text-slate-500' },
  { value: 'struggling' as const, label: 'Hard', icon: Frown, accent: 'text-rose-500' },
];
...
function readMoods(): MoodEntry[] {
  ...
  try { return JSON.parse(localStorage.getItem('neuralcare_moods') ?? '[]'); }
  catch { return []; }
}
...
function WelcomeBanner({ intakeCompleted }: { intakeCompleted: boolean }) {
  ...
  return (
    <section className="relative overflow-hidden rounded-3xl px-5 py-4 text-white bg-gradient-to-br from-teal-500 via-teal-600 to-emerald-700 shadow-[0_8px_32px_rgba(15,118,110,0.30)] border border-white/20 ring-1 ring-inset ring-white/20">
...
function MoodCheckin({
  todayMood, streak, moodHistory, onSelect, disabled,
}: {
    ...
    <Card>
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          {streak > 0 && (
            <Badge variant="secondary" className="text-[9px] font-semibold bg-white/40 text-amber-700 border border-amber-200/60 gap-1 px-1.5 py-0">
  ...
  return (
    <Card>
      <CardContent className="p-4">
        ...
        <div className="h-1 rounded-full bg-white/30 overflow-hidden mb-4">
          <div
            className="h-full rounded-full bg-gradient-to-r from-teal-500 to-emerald-400 transition-all duration-700 ease-out shadow-[0_0_8px_rgba(20,184,166,0.6)]"
            style={{ width: `${pct}%` }}
          />
        </div>
                ...
                <div className="flex flex-col items-center gap-1">
                  <div
                    className={`flex size-8 items-center justify-center rounded-full text-[11px] font-bold transition-all duration-300 ${
                      done    ? 'bg-teal-600 text-white shadow-sm shadow-teal-500/30'
                      : current ? 'bg-white/50 text-teal-700 ring-2 ring-teal-400 shadow-sm'
                  ...
                  <span className={`text-[10px] font-semibold text-center leading-tight ${done ? 'text-teal-700' : current ? 'text-slate-700' : 'text-slate-400'}`}>
                    {step.label}
                  </span>
                </div>
                {i < STEPS.length - 1 && (
                  <div
                    className={`h-[1.5px] flex-1 mx-1.5 sm:mx-3 rounded-full mt-4 transition-colors duration-500 ${done ? 'bg-teal-400/60' : 'bg-white/30'}`}
                    aria-hidden
                  />
                )}
              </li>
...
function QuickActionCard({ href, label, description, icon: Icon, accent, primary, done, locked }: ActionItem) {
  return (
    <Card
      className={`overflow-hidden transition-all duration-200 ${
        locked ? 'opacity-40 grayscale' : 'hover:bg-white/30 hover:border-white/60'
      } ${primary ? 'ring-2 ring-teal-400/50 ring-offset-1' : ''}`}
    >
      <CardContent className="p-0">
        <Link
          href={href}
          className={`flex flex-col justify-between gap-2 p-3.5 h-full min-h-[110px] ${locked ? 'pointer-events-none' : ''}`}
          tabIndex={locked ? -1 : undefined}
          aria-disabled={locked || undefined}
        >
          <div className="flex items-start justify-between">
            <div className={`flex size-9 items-center justify-center rounded-xl bg-white/30 border border-white/50 transition-colors ${locked ? 'text-slate-400' : done ? 'text-emerald-600' : accent}`}>
              {done ? <CheckCircle2 className="size-4" /> : locked ? <Lock className="size-3.5" /> : <Icon className="size-4" />}
            </div>
            {primary && (
              <Badge className="text-[9px] px-1.5 py-0 bg-teal-500/20 text-teal-700 border border-teal-300/50 font-bold shadow-none">
                Start here
              </Badge>
            )}
            {done && (
              <Badge className="text-[9px] px-1.5 py-0 bg-emerald-100/60 text-emerald-700 border border-emerald-200/60 font-bold shadow-none">
                Done
              </Badge>
            )}
          </div>
          ...
          {!locked && (
            <span className="group inline-flex items-center gap-0.5 text-[11px] font-medium text-slate-400 hover:text-teal-600 transition-colors mt-auto">
              {done ? 'Review' : 'Open'}
    ...
    <Card>
      <CardContent className="p-4">
        <div className="flex items-center gap-2 mb-2">
          ...
          <Badge variant="secondary" className="ml-auto text-[9px] font-semibold bg-white/40 text-slate-500 border border-white/60 px-1.5 py-0">
            {tip.category}
          </Badge>
        </div>
  ...
  useEffect(() => {
    fetch('/api/appointments', { credentials: 'include' })
      ...
      .then((data) => {
        ...
        const now = Date.now();
        const future = list
          .filter((a) => a.status !== 'cancelled' && new Date(`${a.date}T${a.timeSlot || '00:00'}`).getTime() >= now)
      ...
      .catch(() => { setUpcomingCount(0); setNextAppt(null); })
    ...
    <div className="min-h-full bg-gradient-to-br from-slate-100 via-white to-slate-50">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-5 space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          <div className="lg:col-span-3"><JourneyProgress completedStep={completedStep} /></div>
          <div className="lg:col-span-2">
            <MoodCheckin todayMood={todayMood} streak={moodStreak} moodHistory={moodHistory} onSelect={handleMoodSelect} disabled={moodSaving} />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
          <section className="lg:col-span-3" aria-label="Next appointment">
            ...
            <div className="mt-2">
              {apptLoading ? <AppointmentSkeleton /> : nextAppt ? <AppointmentCard appointment={nextAppt} /> : <AppointmentEmpty />}
            </div>
          </section>
          <div className="lg:col-span-2"><WellnessTip tip={tip} /></div>
        </div>

        <section aria-label="Quick actions">
          <SectionLabel>Quick actions</SectionLabel>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mt-2">
            {actions.map((action) => <QuickActionCard key={action.href} {...action} />)}
          </div>
        </section>
