    ...
    >
      ...
      <CardContent className="p-5 flex flex-col h-full">
        <div className="flex items-center gap-3 mb-3">
          <div
            className={`flex size-9 items-center justify-center rounded-xl ${plan.iconBg}`}
          >
            <Icon className="size-4.5" />
          </div>
          <div>
            <h3 className="text-sm font-bold text-slate-900">{plan.name}</h3>
            <p className="text-[11px] text-slate-400 leading-tight">{plan.description}</p>
          </div>
        </div>
        ...
        {isFree && <p className="text-[11px] text-slate-400 mb-3">No credit card required</p>}

        <Button
          size="sm"
          disabled={isCurrentPlan}
          className={`w-full rounded-xl font-semibold text-sm h-9 mt-0.5 mb-4 transition-all ${
            plan.popular
        ...
        <div className="space-y-1.5 flex-1">
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.08em] mb-1">
            What&apos;s included
          </p>
...
export function PricingView() {
  ...
  return (
    <div className="h-full flex flex-col bg-[#f8fafb]">
      <div className="flex-1 flex flex-col max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
        <div className="text-center mb-4">
          <Badge className="text-[10px] px-2.5 py-0.5 bg-teal-50 text-teal-700 border border-teal-200 font-bold shadow-none mb-2">
            Pricing
          </Badge>
